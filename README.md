...
# 5. Цели (IP или домен; домен резолвится автоматически). 4-й поле - режим:
#    warp (по умолчанию) | direct | byedpi
echo "192.168.1.50|ТВ|192.168.1.50|warp" >> /etc/warp-targets.conf
sh /usr/bin/warp-targets-apply.sh

# 6. Режим wholelan (весь LAN через WARP): файл /etc/warp-wholelan (1/0) -
#    тумблер есть в дашборде, руками:
echo "1" > /etc/warp-wholelan && sh /usr/bin/warp-targets-apply.sh
...
У каждой цели из `/etc/warp-targets.conf` есть режим — 4-е поле строки
`IP|МЕТКА|ИСТОЧНИК|РЕЖИМ` (меняется кнопкой «Режим» в дашборде):

| Режим | Куда идёт трафик | Механизм |
|---|---|---|
| `warp` (по умолчанию) | в туннель | `ip rule` 150-199 → таблица `warp` → tun0 |
| `byedpi` | в ByeDPI | `ip rule` 75 → таблица `main`, без RETURN: tcp 443/80 попадает в REDIRECT на 1081 |
| `direct` | напрямую в WAN | `ip rule` 75 → таблица `main`, с RETURN от ByeDPI |

Плюс режим **WholeLAN** (тумблер в дашборде, файл `/etc/warp-wholelan`, `1`/`0`):
весь LAN без исключений уходит в WARP через `ip rule` priority 80
`from <SUBNET> lookup warp` (параметры подсети в `/etc/warp-lan.conf`).

Порядок обработки пакета из LAN (важно для понимания):

1. **nat PREROUTING**: если адрес источника = цель с режимом `warp`/`direct`
   (или вся подсеть при WholeLAN) — `RETURN`, мимо ByeDPI. Исключение вставляется
   позицией 2, до цепочки `BYEDPI`.
2. Иначе, если `tcp 443/80` — `REDIRECT` в ByeDPI (1081): десинхронизация, дальше обычный путь.
3. **Решение о маршрутизации** (`ip rule`, от меньшего приоритета к большему):
   - 150-199 — цель `warp` → таблица `warp` → **WARP**;
   - 80 — WholeLAN → таблица `warp` → **WARP**;
   - 75 — цель `direct`/`byedpi` → таблица `main` → **прямой WAN** (ByeDPI или нет — решает RETURN на шаге 1);
   - иначе — таблица `main` → **прямой WAN**.
4. FORWARD/POSTROUTING: MASQUERADE на tun0.

Важные следствия:

- **При включённом WholeLAN режим `byedpi` у целей не работает** — весь LAN-трафик исключён из REDIRECT и уходит в туннель (фактически = `direct`). Режим `direct` при WholeLAN продолжает работать (приоритет 75 выше 80).
- `byedpi` имеет смысл при выключенном WholeLAN: сайты вне списка целей идут напрямую с десинхронизацией, цели `byedpi` — десинхронизация без туннеля, цели `warp` — только туннель.
...
- Правила 90/100 (ручные, из старых экспериментов) установщик не трогает; управляются только 75, 80, 150-199.
- Скрипт `/usr/bin/warp-targets-apply.sh` идемпотентен: при каждом запуске чистит управляемые приоритеты и пересоздаёт из конфига. Молча выходит, если tun0 не поднят (применится по крону позже).
...
**Смена стратегии** — кнопками в дашборде (Статус → «Стратегия A/B/C», применяет флаги в обоих сервисах и перезапускает) или руками:
...
Сначала запусти `/usr/bin/usque --help`: `Segmentation fault` сразу после перезагрузки обычно означает hard-float бинарник на MT7621 без FPU. Используй `core/usque-mipsel` из этого репозитория — он собран с `GOARCH=mipsle GOMIPS=softfloat`. Также проверь `df -h /overlay`: при ~100% нужен `reboot` (jffs2 не освобождает место от удалённых файлов без перемонтирования).
